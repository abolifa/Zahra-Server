                    ...
                    <div class="card-body">
                        ...
                        >
                            ...
                            <b-tab title="Order Items" @click="getOrders">
                                ...
                                <b-row>
                                    <div
                                        class="col-md-12 text-success text-center h6"
                                    >
                                        Total:- {{ $currency }}
...
export default {
    ...
    methods: {
        getOrderStatus: function () {
            let vm = this;
            axios
                .get(this.$apiUrl + "/order_statuses")
                .then((response) => {
        ...
        getOrders() {
            ...
            axios
                ...
                .then((response) => {
                    ...
                    this.total_amount = this.orders
                        .map((item) => Number(item.total))
                        .reduce((prev, curr) => prev + curr, 0)
                        .toFixed(2);
                    this.delivery_charge = this.orders
                        .map((item) => Number(item.delivery_charge))
                        .reduce((prev, curr) => prev + curr, 0)
                        .toFixed(2);
                    this.final_total = this.orders
                        .map((item) => Number(item.final_total))
                        .reduce((prev, curr) => prev + curr, 0)
                        .toFixed(2);

                    this.order_items = response.data.data.order_items;
                    this.totalOrderItemRows = this.order_items.length;
                    this.order_items_total_sum = this.order_items
                        .map((item) => Number(item.total))
                        .reduce((prev, curr) => prev + curr, 0)
                        .toFixed(2);
            ...
            this.$swal
                ...
                .then((result) => {
                    if (result.value) {
                        ...
                        axios
                            .post(this.$apiUrl + "/orders/delete", postData)
                            .then((response) => {
                                this.isLoading = false;
            ...
            this.$swal
                ...
                .then((result) => {
                    if (result.value) {
                        ...
                        axios
                            .post(
                                this.$apiUrl + "/orders/delete_item",
                                postData
                            )
                            .then((response) => {
                                this.isLoading = false;
