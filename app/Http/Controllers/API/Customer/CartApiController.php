    ...
    public function getUserCart(Request $request): JsonResponse
    {
        ...
        if (ProductHelper::isItemAvailableInUserCart($user_id)) {
            ...
            foreach ($res as $key => $row) {
                ...
                $res[$key]->variant_images = CommonHelper::getImages($row->product_id, $row->product_variant_id);
                $res[$key]->product_images = CommonHelper::getImages($row->product_id, 0);
                $res[$key]->main_image = $res[$key]->variant_images[0] ?? $res[$key]->product_images[0] ?? $row->image;
