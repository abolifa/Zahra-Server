...
class ProductApiController extends Controller
{
    ...
    public function getProducts(Request $request)
    {
        ...
        try {
            ...
            $jsonSearch = [];
            if ($request->has('jsonSearch')) {
                $jsonSearch = json_decode($request->jsonSearch, true);
            }
            if (!empty($jsonSearch)) {
                if (isset($jsonSearch['brand']) && is_numeric($jsonSearch['brand'])) {
                    $sql = $sql->where('p.brand_id', intval($jsonSearch['brand']));
                }

                if (isset($jsonSearch['category']) && is_numeric($jsonSearch['category'])) {
                    $sql = $sql->where('p.category_id', intval($jsonSearch['category']));
                }

                if (isset($jsonSearch['discount']) && is_numeric($jsonSearch['discount'])) {
                    $discountValue = intval($jsonSearch['discount']);
                    $sql = $sql->havingRaw("cal_discount_percentage >= ?", [$discountValue]);
                }
            }
                    ...
                    foreach ($variants as $key => $variant) {
                        if ($variant) {
                            $variant = $variant->makeHidden([
                                'product_id', 'stock_unit_id', 'deleted_at',
                                'cal_discount_percentage', 'order_counter'
                            ]);

                            $variantArray = $variant->toArray();

                            // ✅ Stock status
                            $variantArray['status'] = ($variantArray['stock'] <= 0 && $variantArray['is_unlimited_stock'] == 0) ? 0 : intval($variantArray['status']) ?? 0;

                            // ✅ Fetch variant image (from product_images)
                            $image = DB::table('product_images')
                                ->where('product_variant_id', $variantArray['id'])
                                ->value('image');
                            $variantArray['image'] = $image ? asset('storage/' . $image) : '';

                            // ✅ Cart count
                            if ($user_id) {
                                $cart = Cart::where('product_variant_id', $variantArray['id'])
                                    ->where('user_id', $user_id)
                                    ->first();
                                $variantArray['cart_count'] = $cart ? $cart['qty'] : 0;
                            } else {
                                $variantArray['cart_count'] = 0;
                            }

                            // ✅ Taxed prices
                            $taxed = ProductHelper::getTaxableAmount($variantArray['id']);
                            $variantArray['discounted_price'] = CommonHelper::doubleNumber($taxed->taxable_discounted_price ?? $variantArray['discounted_price']);
                            $variantArray['price'] = CommonHelper::doubleNumber($taxed->taxable_price ?? $variantArray['price']);
                            $variantArray['taxable_amount'] = CommonHelper::doubleNumber($taxed->taxable_amount);
                            $variantArray['stock_unit_name'] = $variantArray['stock_unit_name'] ?? '';

                            // ✅ Assign final variant array
                            $variants[$key] = $variantArray;
                        }
                    }
