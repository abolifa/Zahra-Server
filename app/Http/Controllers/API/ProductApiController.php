...
class ProductApiController extends Controller
{
    ...
    public function getProducts(Request $request)
    {
        ...
        pv.id as product_variant_id, pv.price, pv.discounted_price, pv.color, pv.color_code, pv.measurement, pv.status as pv_status , pv.stock,pv.stock_unit_id, u.short_code,
    ...
    public function save(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'measurement.*' =>  ['required_if:type,loose', 'numeric'],
            'price.*' =>  ['required_if:type,loose', 'numeric'],
        ...
        foreach ($request->measurement as $index => $item) {
            $data = array();
            $data['measurement'] = $request->measurement[$index];
            $data['price'] = $request->price[$index];
            $data['discounted_price'] = $request->discounted_price[$index];
            $data['discount_expires'] = $request->discounted_expiry[$index];
            $data['color'] = $request->color[$index];
            $data['color_code'] = $request->color_code[$index];
            $originalPrice = $request->price[$index];
            $discountedPrice = $request->discounted_price[$index];
        ...
        try {
            ...
            // $product->deal_expires = $request->deal_expires;
            ...
            foreach ($request->measurement as $index => $item) {
                ...
                $data['stock_unit_id'] = $request->stock_unit_id;
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->measurement as $index => $item) {
                ...
                $variant->stock = ($request->is_unlimited_stock == 0) ? $request->stock : 0;
                $variant->stock_unit_id = $request->stock_unit_id;
                $variant->status = $request->status;
                $variant->measurement = $request->measurement[$index];
                $variant->price = $request->price[$index];
                $variant->color = $request->color[$index];
                $variant->color_code = $request->color_code[$index];
                $variant->discounted_price = isset($request->discounted_price[$index]) ? $request->discounted_price[$index] : 0;

                $variant->discount_expires = $request->discounted_expiry[$index];
                $originalPrice = $request->price[$index];
                $discountedPrice =  isset($request->discounted_price[$index]) ? $request->discounted_price[$index] : 0;
                $discountPercentage = (($originalPrice - $discountedPrice) / $originalPrice) * 100;

                $variant->discount = $discountPercentage;
