...
class CommonHelper
{
    public static function categoryTreeAsJson($parent_id = 0): array
    {
        $categories = Category::where('parent_id', $parent_id)->get();
        $result = [];

        foreach ($categories as $category) {
            $children = self::categoryTreeAsJson($category->id);
    ...
    public static function slugify($text, $table = 'products', $field = 'slug', $key = NULL, $value = NULL): string
    {
    ...
    public static function getSettings($variables): array
    {
    ...
    public static function categoryTreeHtml(
        $parent_id = 0,
        $sub_mark = '',
        $default = null,
        $dont_include = [],
        $only_last_selectable = false,
        $multiple_default = [],
        $seller_id = 0
    ): string
    {
        $html = '';
        $query = Category::with('childs')->where('parent_id', $parent_id);
        ...
        if (!empty($dont_include)) {
            foreach ($dont_include as $id) {
                $query->where('id', '!=', $id)->where('parent_id', '!=', $id);
            }
        }
        ...
        foreach ($categories as $category) {
            $selected = ($default == $category->id) ? 'selected' : '';
            $multiSelected = (in_array($category->id, $multiple_default)) ? 'selected' : '';
            $hasChildren = $category->childs && $category->childs->count();

            if ($only_last_selectable) {
                if ($hasChildren) {
                    // Render <optgroup> for this parent
                    $html .= '<optgroup label="' . htmlspecialchars($category->name) . '">';
                    // Flatten and render only leaves
                    $html .= self::renderLeafOptions($category->childs, '-- ', $default, $multiple_default, $dont_include);
                    $html .= '</optgroup>';
                } else {
                    // This is a leaf
                    $html .= '<option value="' . $category->id . '" ' . $selected . ' ' . $multiSelected . '>' . htmlspecialchars($category->name) . '</option>';
                }
            } else {
                // Not only_last_selectable: render everything as option recursively
                $html .= '<option value="' . $category->id . '" ' . $selected . ' ' . $multiSelected . '>' . htmlspecialchars($sub_mark . $category->name) . '</option>';
                $html .= self::categoryTreeHtml($category->id, $sub_mark . '-- ', $default, $dont_include, $only_last_selectable, $multiple_default, $seller_id);
            }
        }
    ...
    private static function renderLeafOptions($categories, $sub_mark = '', $default = null, $multiple_default = [], $dont_include = [])
    {
        $html = '';

        foreach ($categories as $category) {
            if (!empty($dont_include) && in_array($category->id, $dont_include)) {
                continue;
            }
            ...
            if ($hasChildren) {
                $html .= self::renderLeafOptions($category->childs, $sub_mark . '-- ', $default, $multiple_default, $dont_include);
            } else {
                $html .= '<option value="' . $category->id . '" ' . $selected . ' ' . $multiSelected . '>' . htmlspecialchars($sub_mark . $category->name) . '</option>';
            }
        }
    ...
    public static function uploadProductImages($images, $product_id, $variant_id = 0): void
    {
    ...
    public static function validatePromoCode($user_id, $promo_code, $total): array
    {
    ...
    public static function getSellerIds($latitude, $longitude): array
    {
        ...
        return array_values(array_column($sellers->toarray(), 'id'));
    }
    ...
    public static function updateUserWalletBalance($new_balance, $id): void
    {
    ...
    public static function addWalletTransaction($order_id, $order_item_id, $user_id, $type, $wallet_balance, $message, $status = 1): false|WalletTransaction
    {
    ...
    public static function isOrderDeliverable($address_id, $latitude, $longitude, $seller_id, $type = "address"): bool
    {
        if (!empty($seller_id) || $seller_id != "") {
            ...
            } else if (!empty($seller) && $seller->geolocation_type == "circle") {
                $distance = $data->distance;
                if ($distance <= $city_distance) {
                    return true;
                } else {
    ...
    public static function isInPolygon($points_polygon, $vertices_x, $vertices_y, $longitude_x, $latitude_y): bool|int
    {
        $i = $j = $c = 0;
    ...
    public static function convertToParent($measurement, $measurement_unit_id): float|int
    {
    ...
    public static function isOrderItemCancelled($order_item_id): bool
    {
    ...
    public static function isOrderItemReturned($active_status, $postStatus): bool
    {
    ...
    public static function getSectionWithProduct($seller_ids, $user_id = 0): array
    {
        $limit = 7;
        $offset = 0;
        ...
        foreach ($sections as $key => $section) {
            Log::info("Loading section: {$section->title}, type: {$section->product_type}");
            if (trim(strtolower($section->product_type)) === 'new_added_products') {
                ...
                $product_ids = $query->orderBy('created_at', 'DESC')
                    ->limit($limit)
                    ->pluck('id')
                    ->implode(',');
            } else {
            ...
            $products = Product::select('p.*', 'p.type as d_type', 's.store_name as seller_name', 's.slug as seller_slug', 's.status as seller_status', 'b.name as brand_name')
                ->from('products as p')
            ...
            $products = $products->makeHidden([
                'seller_id', 'row_order', 'return_status', 'cancelable_status', 'till_status', 'description', 'status',
                'is_approved', 'return_days', 'pincodes', 'cod_allowed', 'pickup_location', 'tags', 'd_type', 'seller_name',
                'seller_slug', 'seller_status', 'created_at', 'updated_at', 'deleted_at', 'image', 'other_images'
            ]);
            $i = 0;
            foreach ($products as $row) {
                $variants = ProductVariant::select('*',
                    ...
                    ->get()
                    ->makeHidden(['product_id', 'status', 'measurement_unit_id', 'stock_unit_id', 'deleted_at']);
                if (empty($variants)) {
                    continue;
                }
                ...
                foreach ($variants as $variant) {
                    $variantArray[] = self::getProductVariant($variant['id'], $user_id);
                }
    ...
    public static function getProductDetails($product_id, $user_id = null, $is_variants = true, $request = null)
    {
        $product = Product::select('products.*', 'sellers.longitude', 'sellers.latitude', 'cities.max_deliverable_distance',
            'cities.boundary_points', 'co.name as country_made_in', 'b.name as brand_name')
    ...
    public static function isDeliverable($max_deliverable_distance, $latitudeFrom, $longitudeFrom, $latitudeTo, $longitudeTo, $unit = 'K'): bool|int
    {
    ...
    public static function getAllDeliveryCharge($latitudeFrom, $longitudeFrom, $seller_ids, $sub_total): array
    {
        $sellers = Seller::select(
            ...
            'sellers.longitude',
            'sellers.city_id',
            DB::raw("6371 * acos(cos(radians(" . $latitudeFrom . "))
            * cos(radians(sellers.latitude))
            * cos(radians(sellers.longitude) - radians(" . $longitudeFrom . "))
            + sin(radians(" . $latitudeFrom . "))
            * sin(radians(sellers.latitude))) AS distance"),
        ...
        if ($sellers->isNotEmpty()) {
            ...
            foreach ($sellers as $seller) {
                $delivery = self::getDeliveryCharge(
                    $latitudeFrom,
                    $longitudeFrom,
                    $seller->latitude,
                    $seller->longitude,
                ...
                if ($delivery["error"] === true) {
                    // fallback: still include seller with 0 charge and mark as not deliverable
                    $data[] = [
    ...
    public static function getDeliveryCharge($latitudeFrom, $longitudeFrom, $latitudeTo, $longitudeTo, $city_id, $sub_total): array
    {
        ...
        $fallbackDistance = function () use ($latitudeFrom, $longitudeFrom, $latitudeTo, $longitudeTo) {
            $earthRadius = 6371;
            $latDelta = deg2rad($latitudeTo - $latitudeFrom);
            $lngDelta = deg2rad($longitudeTo - $longitudeFrom);
        ...
        if (!isset($result['body']) || (isset($result['http_code']) && $result['http_code'] != "200")) {
            ...
            if ($charge_method == "range_wise_charges") {
                ...
                foreach ($ranges as $range) {
                    if ($d >= $range['from_range'] && $d <= $range['to_range']) {
            ...
            return [
                'error' => false,
                'message' => 'Fallback distance used',
                'charge' => $charge,
        ...
        if (isset($result['body']['status']) && $result['body']['status'] == "OK") {
